    ...
    public class ERC1155 : Routable
    {
        ...
        public async Task<List<NFT>> GetOwned(string address = null)
        {
            ...
            else
            {
                ...
                try
                {
                    // Prepare functions for balance calls in parallel
                    var balanceFunctions = Enumerable.Range(0, (int)totalCount)
                        .Select(i => new TokenERC1155Contract.BalanceOfFunction() { Account = owner, Id = new BigInteger(i) }).ToArray();

                    // Perform multicall to get balances in parallel
                    ...
                    // Filter out token IDs with non-zero balances
                    var nonZeroBalanceTokenIds = balanceResults
                        .Select((result, index) => (Balance: result.ReturnValue1, TokenId: index))
                        .Where(x => x.Balance > 0)
                        .ToList();

                    // Prepare URI fetch functions based on token IDs
                    var uriFunctions = nonZeroBalanceTokenIds
                        .Select(x => new TokenERC1155Contract.UriFunction() { TokenId = new BigInteger(x.TokenId) })
                        .ToArray();

                    // Perform multicall to fetch URIs in parallel
                    var uriResults = await TransactionManager.ThirdwebMulticallRead<TokenERC1155Contract.UriFunction, TokenERC1155Contract.UriOutputDTO>(_sdk, _contractAddress, uriFunctions);

                    // Fetch metadata from IPFS concurrently
                    var metadataFetchTasks = uriResults
                        .Select(uriResult => _sdk.Storage.DownloadText<NFTMetadata>(uriResult.ReturnValue1.ReplaceIPFS(_sdk.Storage.IPFSGateway)))
                        .ToList();

                    var metadataResults = await Task.WhenAll(metadataFetchTasks);  // Perform metadata fetch in parallel

                    // Prepare the owned NFTs list
                    ownedNfts = new List<NFT>();
                    var nftCreationTasks = nonZeroBalanceTokenIds.Select(async (item, i) =>
                    {
                        var tokenId = item.TokenId.ToString();
                        var balance = item.Balance;
                        var metadata = metadataResults[i];

                        // Replace IPFS links with proper gateway URLs
                        metadata.image = metadata.image.ReplaceIPFS(_sdk.Storage.IPFSGateway);
                        metadata.id = tokenId;
                        metadata.uri = uriResults[i].ReturnValue1.ReplaceIPFS(_sdk.Storage.IPFSGateway);

                        // Add NFT to the list
                        ownedNfts.Add(new NFT
                        {
                            owner = owner,
                            type = "ERC1155",
                            supply = await TotalSupply(tokenId),
                    ...
                    // Wait for all NFT creations to finish
                    await Task.WhenAll(nftCreationTasks);
                }
                catch
                {
                    // Fallback in case of an error (e.g., Multicall3 not deployed)
                    ThirdwebDebug.LogWarning("Unable to fetch using Multicall3, likely not deployed on this chain, falling back to single queries.");
