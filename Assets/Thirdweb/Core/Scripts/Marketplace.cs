    ...
    public class DirectListings : Routable
    {
        ...
        public async Task<List<DirectListing>> GetAll(MarketplaceFilters filters = null)
        {
            ...
            else
            {
                ...
                // ใช้ Task.WhenAll เพื่อดึงข้อมูล GetListing แบบขนาน
                var listingTasks = allListings.Select(listing => GetListing(listing.ListingId.ToString()));
                var tempListings = await Task.WhenAll(listingTasks);

                // กรองรายการที่ดึงมาให้ตรงกับ filter
                var filteredListings = tempListings
                    .Where(tempListing =>
                        (filters?.tokenContract == null || filters.tokenContract == tempListing.assetContractAddress) &&
                        (filters?.tokenId == null || filters.tokenId == tempListing.tokenId) &&
                        (filters?.seller == null || filters.seller == tempListing.creatorAddress))
                    .ToList();
        ...
        public async Task<List<DirectListing>> GetAllValid(MarketplaceFilters filters = null)
        {
            ...
            {
                // Retrieve the total count and calculate the start and end points
                int totalSupply = int.Parse(await GetTotalCount());
                int start = filters?.start ?? 0;
                int count = filters?.count ?? 0;
                int end = count == 0 ? totalSupply - 1 : start + count;

                // Fetch all valid listings in the given range
                ...
                // Use parallel tasks to speed up fetching individual listings
                var tasks = allValidListings.Select(listing => GetListing(listing.ListingId.ToString()));
                var tempListings = await Task.WhenAll(tasks);

                // Apply filtering logic to only include relevant listings based on the filters
                var filteredListings = tempListings
                    .Where(tempListing =>
                        (filters?.tokenContract == null || filters.tokenContract == tempListing.assetContractAddress) &&
                        (filters?.tokenId == null || filters.tokenId == tempListing.tokenId) &&
        ...
        public async Task<DirectListing> GetListing(string listingID)
        {
            ...
            else
            {
                ...
                // Use Task.WhenAll to fetch currency and metadata in parallel
                var currencyTask = _sdk.GetContract(result.Listing.Currency).ERC20.Get();

                // Attempt to fetch metadata from ERC721, and fallback to ERC1155 if it fails
                var metadataTask = GetMetadataAsync(result.Listing.AssetContract, result.Listing.TokenId.ToString());

                // Wait for both tasks to complete
                await Task.WhenAll(currencyTask, metadataTask);

                // Extract the results
                Currency currency = await currencyTask;
                var metadata = await metadataTask;

                // Return the DirectListing object with all the information
        ...
        // Helper method for fetching metadata
        private async Task<NFTMetadata> GetMetadataAsync(string assetContract, string tokenId)
        {
            try
            {
                // Attempt to get ERC721 metadata
                var metadata = (await _sdk.GetContract(assetContract).ERC721.Get(tokenId)).metadata;
                return metadata;
            }
            catch
            {
                // Fallback to ERC1155 if ERC721 fails
                var metadata = (await _sdk.GetContract(assetContract).ERC1155.Get(tokenId)).metadata;
                return metadata;
            }
        }
                ...
                return await TransactionManager.ThirdwebWrite(
                    ...
                    new DirectListingsContract.CreateListingFunction()
                    {
                        Params = new DirectListingsContract.ListingParameters()
                        {
                            ...
                            PricePerToken = BigInteger.Parse(input.pricePerToken.ToWei()) / BigInteger.Pow(10, 12),
