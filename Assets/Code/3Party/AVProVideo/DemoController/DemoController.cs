...
namespace RenderHeads.Media.AVProVideo.Demos
{
	public class DemoController : MonoBehaviour
	{
		...
		void OnGUI()
		{
			if (SceneManager.GetActiveScene().buildIndex < _demoSceneStart || SceneManager.GetActiveScene().buildIndex > _demoSceneEnd)
			{
				return;
			}
			...
			if( SceneManager.GetActiveScene().buildIndex > 1 && SceneManager.GetActiveScene().buildIndex < _exitSceneIndex )
			{
			...
			if( SceneManager.GetActiveScene().buildIndex == _demoSceneEnd )
			{
		...
		void OnLevelFinishedLoading(Scene scene, LoadSceneMode mode)
		{
			...
			StartCoroutine( WaitForVideoLoad( scene.buildIndex ) );
		}
