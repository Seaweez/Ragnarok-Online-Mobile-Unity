...
public class UILabel : UIWidget
{
    ...
    {
        if (LocalizationManager.Instance != null && !string.IsNullOrEmpty(this.text))
        {
            // ดึงข้อความแปลจาก LocalizationManager โดยใช้ข้อความที่มีอยู่ในช่อง text เป็นคีย์
            string localizedText = LocalizationManager.Instance.GetLocalizedText(this.text);

            if (!string.IsNullOrEmpty(localizedText))
            {
                this.text = localizedText; // ตั้งค่าข้อความแปลใน UILabel
               // Debug.Log("Text translated to: " + localizedText);
            }
            else
            {
                //Debug.LogWarning("Localization key not found for text: " + this.text);
            }
        }
        else
        {
           // Debug.LogError("LocalizationManager instance not found or text is empty.");
        }
    }
