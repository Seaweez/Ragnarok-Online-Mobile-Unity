...
public static class Localization
{
	...
	static public Dictionary<string, string[]> dictionary
	{
		get
		{
			if (!localizationHasBeenSet) LoadDictionary(PlayerPrefs.GetString("Language", "WW_English"));
			return mDictionary;
		}
		set
		{
	...
	static public string language
	{
		get
		{
			...
			{
				mLanguage = PlayerPrefs.GetString("Language", "WW_English");
    ...
    static bool LoadDictionary(string value)
    {
        if (!localizationHasBeenSet)
        {
            ...
            if (asset != null)
            {
                try
                {
                    // Deserialize the JSON data into a temporary dictionary
                    var tempDict = JsonConvert.DeserializeObject<Dictionary<string, string>>(asset.text);
                    if (tempDict != null)
                    {
                        // Convert each string to a single-element array and populate mDictionary
                        mDictionary = new Dictionary<string, string[]>();
                        foreach (var pair in tempDict)
                        {
                            mDictionary.Add(pair.Key, new string[] { pair.Value });
                        }

                        // Populate mLanguages if it hasn't been set yet
                        if (mLanguages == null)
                        {
                            mLanguages = new string[] { value };
                        }
                        else if (!mLanguages.Contains(value))
                        {
                            var langList = new List<string>(mLanguages) { value };
                            mLanguages = langList.ToArray();
                        }

                        // Localization data has been set
                        localizationHasBeenSet = true;

                        // Invoke the localization notification if it has been subscribed to
                        onLocalize?.Invoke();
                ...
                {
                    Debug.LogError($"Error parsing localization JSON for language: {value} - Exception: {ex.Message}");
                }
            }
            else
            {
                Debug.LogError($"Failed to load localization asset for language: {value}");
            }
        }
