	...
	static bool MakeTextureAnAtlas (string path, bool force, bool alphaTransparency)
	{
			...
			settings.npotScale != TextureImporterNPOTScale.ToNearest)
		{
            ...
            if (NGUISettings.trueColorAtlas)
            {
                ...
                // Check if the texture is intended for GUI use and set an appropriate format
                if (ti.textureType == TextureImporterType.GUI)
                {
                    platSetting.format = TextureImporterFormat.RGBA32; // Use RGBA32 for GUI textures
                }
                else
                {
                    platSetting.format = TextureImporterFormat.ARGB32; // Otherwise, use ARGB32 or another suitable format
                }
