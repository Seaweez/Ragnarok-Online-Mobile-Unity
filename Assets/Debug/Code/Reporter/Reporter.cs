...
public class Reporter : SingleTonGO<Reporter> {
	...
	#if USE_OLD_UNITY
	Application.LogCallback captureLog_CallBack = new Application.LogCallback (CaptureLog);
	Application.LogCallback captureLogThread_CallBack = new Application.LogCallback (CaptureLogThread);
	#endif
	...
	protected override void OnDestroy ()
	{
		...
		#if USE_OLD_UNITY
//			Application.UnRegisterLogCallback (captureLog_CallBack);
//			Application.UnRegisterLogCallbackThreaded (captureLogThread_CallBack);
		#else
			if(Initialized)
			{
	...
	public void Initialize()
	{
		if( !created )
		{
			...
			#else
			scenes = new string[ 999 ];
			#endif
			currentScene = Application.loadedLevelName;
			DontDestroyOnLoad( gameObject );
#if USE_OLD_UNITY
			Application.RegisterLogCallback (captureLog_CallBack);
			Application.RegisterLogCallbackThreaded (captureLogThread_CallBack);
#else
			Application.logMessageReceived += CaptureLog ;
			Application.logMessageReceivedThreaded += CaptureLogThread ;
#endif
...
//		}
	...
	void OnLevelFinishedLoading(Scene scene, LoadSceneMode mode)
	{
		...
		currentScene = Application.loadedLevelName ;
		LoggerUnused.LogFormat( "Scene {0} is loaded", Application.loadedLevelName);
	}
